...
function myAlphabetLenght(array) {
...
for (data of wowDatatypes) {
  console.log(
    "Item: " +
      data +
      ", Index: " +
      wowDatatypes.indexOf(data) +
      ", Type: " +
      typeof data
  );
}
...
let commonCourse = [];
for (course of student1Courses) {
  commonCourse += student2Courses.filter(
    (student2Course) => student2Course == course
  );
}
...
for (stuff of furniture) {
  console.log("Letters in '" + stuff + "': " + stuff.split(""));
}
...
console.log(temperatures.filter((temperature) => temperature > 0));
